...
import { installVersionOnDevices } from "../lib/api"
...
export function InstallVersionModal({ version, onlineDevices, onClose, onSuccess }: InstallVersionModalProps) {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      const result = await installVersionOnDevices(version.id, Array.from(selectedDevices))
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Install Version {version.version_code}</DialogTitle>
          <DialogDescription>
            Select devices to install {version.version_name || `version ${version.version_code}`}
          </DialogDescription>
        </DialogHeader>
