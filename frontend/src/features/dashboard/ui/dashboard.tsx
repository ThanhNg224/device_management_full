...
import { DeviceLogsTable } from "../../logs/ui/device-logs-table"
import { VersionsTable } from "../../versions/ui/versions-table"
...
function DashboardContent() {
  ...
  React.useEffect(() => {
    const loadData = async () => {
      try {
        setError(null)
        const res = await devicesController.listDevices()
        ...
        <main className="flex-1 overflow-auto p-6">
          {loading ? (
            ...
            <>
              ...
              {activeTab === "versions" && <VersionsTable onlineDevices={devices.filter(d => d.status === "Online")} />}
            </>
          )}
        </main>
      </SidebarInset>
